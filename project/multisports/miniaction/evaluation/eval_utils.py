...
def build_tubes(anno, frm_dets, K=1):
    ...
    for v in track(vlist, description='building tubes...'):
        ...
        for ilabel in range(len(dataset.labels)):
            ...
            for frame in range(1, dataset.nframes[v] + 2 - K):
                ...
                ltubelets = np.array(
                    VDets[frame][ilabel]
                )  # [:,range(4*K) + [4*K + 1 + ilabel]]  Nx(4K+1) with (x1 y1 x2 y2)*K ilabel-score
...
def frameAP(GT, alldets, thr, print_info=True):
    ...
    for ilabel, label in enumerate(GT['labels']):
        ...
        if label in [
            ...
            continue
        # det format: <video_index><frame_number><label_index><score><x1><y1><x2><y2>
        ...
        # pr will be an array containing precision-recall values， 以每一个样本的score作为阈值，得到的pr数据
        ...
        ):  # traverse each det bbox in class `label` according to the score
            k = (int(detections[j, 0]), int(detections[j, 1])
                 )  # k -> (video_idx, frame_idx)
...
def videoAP_all(groundtruth, detections):
    ...
    map = {
        f'v_map_0.05:0.45': round(low_ap, 4),
        f'v_map_0.10:0.90': round(all_ap, 4),
        f'v_map_0.50:0.95': round(high_ap, 4),
    }
    return map
...
def videoAP_error(GT, alldets, thr):
    ...
    print('')
    print(
        '{:20s} {:8s} {:8s} {:8s} {:8s} {:8s} {:8s} {:8s} {:8s} {:8s} {:8s} {:8s}'
...
if __name__ == '__main__':
    ...
    parser.add_argument('task', help='evaluate type.')
    parser.add_argument('detections', help='pkl file containing detections.')
    parser.add_argument(
        '--groundtruth',
