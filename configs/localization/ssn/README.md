...
4. The reference performance is evaluated with `ResNet50` backbone, while the initial performance is evaluated with `BNInception` backbone. That is the reason why there is a performance drop. The comparison of MMAction2 SSN and the reference SSN are carried out under the same setting and data.
