...
SPDX-License-Identifier: BSD-3-Clause * For full license text,
see LICENSE.txt file in the repo root or
https://opensource.org/licenses/BSD-3-Clause * By Junnan Li * Based on
huggingface code base * https://github.com/huggingface/transformers/blob/v4
.15.0/src/transformers/models/bert """
    ...
    def forward(
        ...
        if mode == 'mlr':
            ...
            assert encoder_hidden_states is not None, \
                'encoder_hidden_states must be given for cross-attention ' \
                'layers '
            ...
            if mode == 'multimodal':
                assert encoder_hidden_states is not None, \
                    'encoder_hidden_states must be given for ' \
...
class BertModel(BertPreTrainedModel):
    ...
    def get_extended_attention_mask(self, attention_mask: Tensor,
        ...
        Arguments: attention_mask (:obj:`torch.Tensor`): Mask with ones
        indicating tokens to attend to, zeros for tokens to ignore.
        input_shape (:obj:`Tuple[int]`): The shape of the input to the
        model. device: (:obj:`torch.device`): The device of the input to the
        model.

        Returns: :obj:`torch.Tensor` The extended attention mask, with the
        same dtype as :obj:`attention_mask.dtype`.
        """
        ...
        if attention_mask.dim() == 3:
            ...
            # to the padding mask - if the model is an encoder, make the
            # mask broadcastable to [batch_size, num_heads, seq_length,
            # seq_length]
            if is_decoder:
                ...
                # ones mask to the causal mask causal and attention masks
                # must have same type with pytorch version < 1.3
    ...
    def forward(
        ...
        in the cross-attention if the model is configured as a decoder.
        encoder_attention_mask (:obj:`torch.FloatTensor` of shape :obj:`(
        batch_size, sequence_length)`, `optional`): Mask to avoid performing
        attention on the padding token indices of the encoder input. This
        mask is used in the cross-attention if the model is configured as a
        decoder. Mask values selected in ``[0, 1]``: - 1 for tokens that are
        **not masked**, - 0 for tokens that are **masked**. past_key_values
        ...
        embed_size_per_head)`): Contains precomputed key and value hidden
        states of the attention blocks. Can be used to speed up decoding. If
        :obj:`past_key_values` are used, the user can optionally input only
        the last :obj:`decoder_input_ids` (those that don't have their past
        key value states given to this model) of shape :obj:`(batch_size,
        1)` instead of all :obj:`decoder_input_ids` of shape :obj:`(
        batch_size, sequence_length)`. use_cache (:obj:`bool`, `optional`):
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            raise ValueError(
                'You have to specify either input_ids or inputs_embeds or '
                'encoder_embeds ')
        ...
        # head attention_probs has shape bsz x n_heads x N x N input
        # head_mask has shape [num_heads] or [num_hidden_layers x num_heads]
...
class BertLMHeadModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        mask is used in the cross-attention if the model is configured as a
        decoder. Mask values selected in ``[0, 1]``: - 1 for tokens that are
        **not masked**, - 0 for tokens that are **masked**. labels (
        :obj:`torch.LongTensor` of shape :obj:`(batch_size,
        sequence_length)`, `optional`): Labels for computing the
        left-to-right language modeling loss (next word prediction). Indices
        ...
        labels n ``[0, ..., config.vocab_size]`` past_key_values (
        :obj:`tuple(tuple(torch.FloatTensor))` of length
        :obj:`config.n_layers` with each tuple having 4 tensors of shape
        :obj:`(batch_size, num_heads, sequence_length - 1,
        embed_size_per_head)`): Contains precomputed key and value hidden
        ...
        If set to :obj:`True`, :obj:`past_key_values` key value states are
        returned and can be used to speed up decoding (see
        :obj:`past_key_values`). Returns: Example:: >>> from transformers
        import BertTokenizer, BertLMHeadModel, BertConfig >>> import torch
        >>> tokenizer = BertTokenizer.from_pretrained('bert-base-cased') >>>
        config = BertConfig.from_pretrained("bert-base-cased") >>> model =
        BertLMHeadModel.from_pretrained('bert-base-cased', config=config)
