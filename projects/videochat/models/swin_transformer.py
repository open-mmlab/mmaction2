...
class WindowAttention(nn.Module):
    ...
    def __init__(self,
        ...
        relative_coords[:, :, 0] += \
            self.window_size[0] - 1  # shift to start from 0
...
class SwinTransformerBlock(nn.Module):
    ...
    Args: dim (int): Number of input channels. input_resolution (tuple[
    int]): Input resolution. num_heads (int): Number of attention heads.
    window_size (int): Window size. shift_size (int): Shift size for SW-MSA.
    mlp_ratio (float): Ratio of mlp hidden dim to embedding dim. qkv_bias (
    bool, optional): If True, add a learnable bias to query, key, value.
    Default: True qk_scale (float | None, optional): Override default qk
    scale of head_dim ** -0.5 if set. drop (float, optional): Dropout rate.
    Default: 0.0 attn_drop (float, optional): Attention dropout rate.
    Default: 0.0 drop_path (float, optional): Stochastic depth rate.
    Default: 0.0 act_layer (nn.Module, optional): Activation layer. Default:
    nn.GELU norm_layer (nn.Module, optional): Normalization layer.  Default:
    nn.LayerNorm
    """
...
class BasicLayer(nn.Module):
    ...
    Args: dim (int): Number of input channels. input_resolution (tuple[
    int]): Input resolution. depth (int): Number of blocks. num_heads (int):
    Number of attention heads. window_size (int): Local window size.
    mlp_ratio (float): Ratio of mlp hidden dim to embedding dim. qkv_bias (
    bool, optional): If True, add a learnable bias to query, key, value.
    Default: True qk_scale (float | None, optional): Override default qk
    scale of head_dim ** -0.5 if set. drop (float, optional): Dropout rate.
    ...
    depth rate. Default: 0.0 norm_layer (nn.Module, optional): Normalization
    layer. Default: nn.LayerNorm downsample (nn.Module | None, optional):
    Downsample layer at the end of the layer. Default: None use_checkpoint (
    ...
    def extra_repr(self) -> str:
        return f'dim={self.dim}, ' \
               f'input_resolution={self.input_resolution}, depth={self.depth}'
...
class PatchEmbed(nn.Module):
    ...
    Default: 3. embed_dim (int): Number of linear projection output
    channels. Default: 96. norm_layer (nn.Module, optional): Normalization
    layer. Default: None
    """
...
class SwinTransformer(nn.Module):
    r""" Swin Transformer A PyTorch impl of : `Swin Transformer:
    ...
    Stochastic depth rate. Default: 0.1 norm_layer (nn.Module):
    Normalization layer. Default: nn.LayerNorm. ape (bool): If True,
    add absolute position embedding to the patch embedding. Default: False
    patch_norm (bool): If True, add normalization after patch embedding.
    Default: True use_checkpoint (bool): Whether to use checkpointing to
    ...
    def forward(self, x, idx_to_group_img=None, image_atts=None, **kwargs):
        ...
        if idx_to_group_img is None:
            ...
            x_bs = torch.gather(
                x,
                dim=0,
                index=idx_to_group_img.view(-1, 1,
                                            1).expand(-1, x.shape[1],
                                                      x.shape[2]))
            weights = image_atts[:, 1:].unsqueeze(2)  # B L 1
            x_bs_cls = torch.sum(
                (weights * x_bs).transpose(1, 2), dim=-1,
                keepdim=True)  # B C 1
            x_bs_cls = x_bs_cls / torch.sum(
                weights.transpose(1, 2), dim=-1, keepdim=True)  # avgpool
...
def interpolate_relative_pos_embed(rel_pos_bias, dst_num_pos, param_name=''):
    # from: https://github.com/microsoft/unilm/blob
    # /8a0a1c1f4e7326938ea7580a00d56d7f17d65612/beit/run_class_finetuning.py
    # #L348
    ...
    if src_size != dst_size:
        print('Position interpolate %s from %dx%d to %dx%d' %
