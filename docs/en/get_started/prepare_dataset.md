...
MMAction2 supports many existing datasets. In this chapter, we will lead you to prepare dataset for MMAction2.
...
MMAction2 already supports many datasets, we provide shell scripts for data preparation under the path `$MMACTION2/tools/data/`, please refer to [supported datasets](../supported_datasets.md) for details to prepare specify dataset.
...
- Build from RGB video data
  ...
  You need to extract keypoints data from video and convert it to supported format, we provide a [tutorial](/configs/skeleton/posec3d/custom_dataset_training.md) with detailed instructions.
...
- groundtruth bbox
  ...
  `lt_x`: The normalized x-coordinate of left top point of bounding box
  `lt_y`: The normalized y-coordinate of left top point of bounding box
  `rb_y`: The normalized x-coordinate of right bottom point of bounding box
  `rb_y`: The normalized y-coordinate of right bottom point of bounding box
...
- proposal bbox
  proposal bbox is a pickle file generated by person detector, usually needs to be fine-tuned on the target dataset. The pickle file contains a dict with below data structure:
