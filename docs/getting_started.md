...
1. Recognize the action from web camera as input by using a TSN model on cpu,
    sampling 1 frame at a time to inference, averaging the score per 5 times and outputting result labels
    with score higher than 0.2.
...
python demo/webcam_demo.py configs/recognition/tsn/tsn_r50_video_inference_1x1x3_100e_kinetics400_rgb.py \
  ...
  --average-size 5 --threshold 0.2 --device cpu
```
