...
- `--onnx`: If specified, recognition results will be generated by onnx model and `CHECKPOINT_FILE` should be onnx model file path. Onnx model files are generated by `/tools/pytorch2onnx.py`. For now, only support single-gpu mode and fixed input shape. Please note that dataset and onnx model should share the same batch size.
- `--tensorrt`: If specified, recognition results will be generated by TensorRT engine and `CHECKPOINT_FILE` should be TensorRT engine file path. TensorRT engines files are generated by exported onnx models and TensorRT official convertion tools. For now, only support single-gpu mode and fixed input shape. Please note that dataset and TensorRT engine should share the same batch size.
