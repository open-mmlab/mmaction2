...
1. First, write your own LrUpdaterHook somewhere in the codebase. The snippet followed is an example of cumtomized lr updater that uses learning rate based on a specific learning rate ratio: `lrs`, by which the learing rate decreases at each `steps`:
```python
...
class RelativeStepLrUpdaterHook(LrUpdaterHook):
    ...
    def get_lr(self, runner, base_lr):
        # Only this function is mendatory to override
