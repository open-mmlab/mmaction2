...
class TestInference(TestCase):
    ...
    def test_detection_inference(self):
        ...
        det_ckpt = 'http://download.openmmlab.com/mmdetection/v2.0/' \
                   'faster_rcnn/faster_rcnn_r50_fpn_2x_coco/faster_rcnn_r50_fpn_2x_coco_bbox_mAP-0.384_20200504_210434-a5d8aa15.pth'  # noqa: E501
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            results, data_samples = detection_inference(
                config_file, det_ckpt, frm_paths)
            self.assertTrue(results[0].shape, (4, ))
            self.assertIsInstance(data_samples[0], DetDataSample)
            # test with_score
            results, data_samples = detection_inference(
                config_file, det_ckpt, frm_paths, with_score=True)
            self.assertTrue(results[0].shape, (5, ))
            # test inference with model object
            model = init_detector(config=det_config, checkpoint=det_ckpt)
            results, data_samples = detection_inference(model, None, frm_paths)
            self.assertTrue(results[0].shape, (4, ))
            self.assertIsInstance(data_samples[0], DetDataSample)
    ...
    def test_pose_inference(self):
        ...
        pose_ckpt = 'https://download.openmmlab.com/mmpose/top_down/hrnet/' \
        ...
        with TemporaryDirectory() as tmpdir:
            frm_paths, _ = frame_extract(video_path, out_dir=tmpdir)
            det_results, _ = detection_inference(det_config, det_ckpt,
                                                 frm_paths)

            results, data_samples = pose_inference(pose_config, pose_ckpt,
                                                   frm_paths, det_results)
            ...
            # test inference with model object
            model = init_model(config=pose_config, checkpoint=pose_ckpt)
            results, data_samples = pose_inference(model, None, frm_paths,
                                                   det_results)
            assert_dict_has_keys(results[0], ('keypoints', 'bbox_scores',
                                              'bboxes', 'keypoint_scores'))
            self.assertIsInstance(data_samples[0], PoseDataSample)
