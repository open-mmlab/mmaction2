...
def generate_dummy_data(batch_size, input_shape):
    data = {'inputs': [torch.randint(0, 255, input_shape)
...
def test_data_preprocessor():
    with pytest.raises(ValueError):
        ActionDataPreprocessor(mean=[1, 1], std=[0, 0],
                               format_shape='NCTHW_Heatmap')
    with pytest.raises(ValueError):
        psr = ActionDataPreprocessor(format_shape='NCTHW_Heatmap',
                                     to_rgb=True)
        psr(generate_dummy_data(1, (3, 224, 224)))

    raw_data = generate_dummy_data(2, (1, 3, 8, 224, 224))
    psr = ActionDataPreprocessor(mean=[123.675, 116.28, 103.53],
    ...
    psr = ActionDataPreprocessor(format_shape='NCTHW',
                                 to_rgb=True)
    data = psr(deepcopy(raw_data))
    ...
    data = psr(deepcopy(raw_data))
    assert_array_equal(data['inputs'][0],
                       (raw_data['inputs'][0][:, [2, 1, 0]] - psr.mean) / psr.std)
    assert_array_equal(data['inputs'][1],
                       (raw_data['inputs'][1][:, [2, 1, 0]] - psr.mean) / psr.std)
