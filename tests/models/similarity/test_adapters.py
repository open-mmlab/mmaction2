...
import torch
import pytest
from mmaction.models import TransformerAdapter, SimpleMeanAdapter
...
def test_transformer_adapter():
    ...
    with pytest.raises(RuntimeError):
        num_segs_model = 8
        num_segs_featutes = 9
        adapter = TransformerAdapter(num_segs=num_segs_model,
                                     transformer_width=64,
                                     transformer_heads=8,
                                     transformer_layers=2)
        features = torch.randn(2, num_segs_featutes, 64)
        adapted_features = adapter(features)

    num_segs = 8
    adapter  = TransformerAdapter(num_segs=num_segs,
                       transformer_width=64,
                       transformer_heads=8,
                       transformer_layers=2)
