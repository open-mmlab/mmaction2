...
def test_ssn_loss():
    ...
    assert torch.equal(output_loss['loss_activity'].detach().numpy(),
                       output_activity_loss.numpy())
    assert torch.equal(output_loss['loss_completeness'].detach().numpy(),
                       output_activity_loss.numpy() * 0.1)
    assert torch.equal(output_loss['loss_reg'].detach().numpy(),
