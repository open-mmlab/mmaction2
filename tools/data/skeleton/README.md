...
For Kinetics400, since the skeleton annotations are large, we do not provide the direct download links on aliyun. Please use the following link to download the `kpfiles` and extract it under `$MMACTION2/data/skeleton/kpfiles` for Kinetics400 training & testing: https://opendatalab.com/Kinetics400-skeleton
