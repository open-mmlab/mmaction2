...
def non_max_suppression(boxlist,
    ...
    with already selected boxes. In each iteration, the detected bounding box
    with highest score in the available pool is selected.
    ...
    Args:
      ...
      score_threshold: minimum score threshold. Remove the boxes with scores
                       less than this value. Default value is set to -10. A
                       very low threshold to pass pretty much all the boxes,
                       unless the user sets a different score threshold.
...
def multi_class_non_max_suppression(boxlist, score_thresh, iou_thresh,
    ...
    Args:
      boxlist: BoxList holding N boxes.  Must contain a 'scores' field
        ...
        be 1 dimensional (in the case of a single class) or 2-dimensional,
...
def prune_non_overlapping_boxes(boxlist1, boxlist2, minoverlap=0.0):
    """Prunes the boxes in boxlist1 that overlap less than thresh with
    boxlist2.
...
def prune_outside_window(boxlist, window):
    ...
    Returns:
      ...
      valid_indices: a tensor with shape [M_out] indexing the valid bounding
        boxes in the input tensor.
    """
...
def concatenate(boxlists, fields=None):
    ...
    This op concatenates a list of input BoxLists into a larger BoxList. It
