...
class ASFormer(BaseModel):
    ...
    def forward(self, inputs, data_samples, mode, **kwargs):
        ...
        input = torch.zeros(
            len(inputs),
            np.shape(inputs[0])[0],
            max(tensor.size(1) for tensor in inputs),
            dtype=torch.float)
        for i in range(len(inputs)):
            input[i, :, :np.shape(inputs[i])[1]] = inputs[i]
    ...
    def predict(self, batch_inputs, batch_data_samples, **kwargs):
        ...
        torch.save(self.model,
                   'D:/2013jts_hero/video_seg/ASFormer-main/epoch-1200.pth')
...
def exponential_descrease(idx_decoder, p=3):  # 从e的0到e的-6
        ...
        padding_mask = torch.cat([
            padding_mask[:, :, i * self.bl:(i + 1) * self.bl +
                         (self.bl // 2) * 2] for i in range(nb)
                                 ...
                                 dim=0)
        final_mask = self.window_mask.repeat(m_batchsize * nb, 1,
                                             1) * padding_mask
