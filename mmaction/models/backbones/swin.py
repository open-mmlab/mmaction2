...
def window_partition(x: Tensor, window_size: Sequence[int]) -> Tensor:
    ...
    Args:
        ...
        window_size (Sequence[int]): window size, ``(window_size_d, window_size_h, window_size_w)``.

    Returns:
        windows (Sequence[int]): ``(B*num_windows, window_size_d*window_size_h*window_size_w, C)``.
    """
    B, D, H, W, C = x.shape
...
def window_reverse(windows: Tensor, window_size: Sequence[int],
                   B: int, D: int, H: int, W: int) -> Tensor:
    """
    Args:
        windows (Tensor): Input feature windows ``(B*num_windows, window_size_d, window_size_h, window_size_w, C)``.
        window_size (Sequence[int]): window size, ``(window_size_d, window_size_h, window_size_w)``.
...
def get_window_size(x_size: Sequence[int], window_size: Sequence[int],
                    shift_size: Optional[Sequence[int]] = None) -> Union[Tuple[int], Tuple[Tuple[int]]]:
...
class WindowAttention3D(nn.Module):
    ...
    Args:
        ...
        window_size (Sequence[int]): The temporal length, height and width of the window.
    ...
    def __init__(self, dim: int, window_size: Sequence[int], num_heads: int,
        ...
        self.relative_position_bias_table = nn.Parameter(  # 2*Wd-1 * 2*Wh-1 * 2*Ww-1, nH
    ...
    def forward(self, x: Tensor, mask: Optional[Tensor] = None) -> Tensor:
        ...
        Args:
            ...
            mask (Tensor, optional): (0/-inf) mask with shape of ``(num_windows, N, N)``.
                Defaults to None.
        """
        B_, N, C = x.shape
...
class SwinTransformerBlock3D(nn.Module):
    ...
    Args:
        ...
        shift_size (Sequence[int]): Shift size for SW-MSA or W-MSA. Defaults to ``(0, 0, 0)``.
    ...
    def __init__(self, dim: int, num_heads: int, window_size: Sequence[int] = (8, 7, 7),
                 shift_size: Sequence[int] = (0, 0, 0), mlp_ratio: float = 4., qkv_bias: bool = True,
                 qk_scale: Optional[float] = None, drop: float = 0., attn_drop: float = 0.,
                 drop_path: float = 0.1, act_layer: nn.Module = nn.GELU,
                 norm_layer: nn.Module = nn.LayerNorm, use_checkpoint: bool = False) -> None:
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
    ...
    def forward_part1(self, x: Tensor, mask_matrix: Tensor) -> Tensor:
        """Forward function part1 including ``LN`` and ``W-MSA`` or ``SW-MSA``.
        """
        B, D, H, W, C = x.shape
...
def compute_mask(D: int, H: int, W: int, window_size: Sequence[int],
                 shift_size: Sequence[int], device: Union[str, torch.device]) -> Tensor:
...
class BasicLayer(nn.Module):
    ...
    Args:
        ...
        window_size (Sequence[int]): Local window size. Defaults to ``(8, 7, 7)``.
        ...
        drop (float): Dropout rate. Defaults to 0.0.
        attn_drop (float): Attention dropout rate. Defaults to 0.0.
        drop_path (float or Sequence[float]): Stochastic depth rate. Defaults to 0.0.
        act_layer (nn.Module): Activation layer. Defaults to ``nn.GELU``.
        norm_layer (nn.Module): Normalization layer. Defaults to ``nn.LayerNorm``.
...
class PatchEmbed3D(nn.Module):
    ...
    Args:
        patch_size (Sequence[int] or int): Patch token size. Defaults to ``(2, 4, 4)``.
...
class SwinTransformer3D(nn.Module):
    ...
    Args:
        ...
        embed_dim (int): Number of linear projection output channels. Defaults to 96.
        depths (Sequence[int]): Depths of each Swin Transformer stage. Defaults to ``(2, 2, 6, 2)``.
        num_heads (Sequence[int]): Number of attention head of each stage. Defaults to ``(3, 6, 12, 24)``.
        window_size (Sequence[int]): Window size. Defaults to ``(8, 7, 7)``.
        mlp_ratio (float): Ratio of mlp hidden dim to embedding dim. Defaults to 4.
        qkv_bias (bool): If True, add a learnable bias to query, key, value. Defaults to True.
    ...
    def inflate_weights(self, logger: MMLogger) -> None:
        ...
        Args:
            logger (MMLogger): The logger used to print debugging infomation.
        """
        ...
        state_dict['patch_embed.proj.weight'] = state_dict['patch_embed.proj.weight']. \
                                                    unsqueeze(2).repeat(1, 1, self.patch_size[0], 1, 1) / \
                                                self.patch_size[0]
            ...
            if nH1 != nH2:
                ...
                if L1 != L2:
                    ...
                    relative_position_bias_table_pretrained_resized = torch.nn.functional.interpolate(
                        ...
                        mode='bicubic')
                    relative_position_bias_table_pretrained = relative_position_bias_table_pretrained_resized. \
