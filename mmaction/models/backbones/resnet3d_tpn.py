...
class AuxHead(nn.Module):
    ...
    def forward(self, x, target=None):
        ...
        loss['loss_aux'] = self.loss_weight * self.loss_cls(x, target)
        return loss
...
class ResNet3dTPN(ResNet3dSlowOnly):

    def __init__(self,
                 *args,
                 tpn_in_channels=(256, 512, 1024, 2048),
                 tpn_out_channels=256,
                 ...
                 aux_head_cfg=None,
                 flow_type='cascade'):
        super(ResNet3dTPN, self).__init__(*args)

        assert isinstance(tpn_in_channels, tuple)
        assert isinstance(tpn_out_channels, int)
        self.tpn_in_channels = tpn_in_channels
        self.tpn_out_channels = tpn_out_channels
        self.num_tpn_stages = len(tpn_in_channels)
        ...
        assert level_fusion_cfg is None or isinstance(aux_head_cfg, dict)
        ...
        for i in range(self.num_tpn_stages):
            in_channels = tpn_in_channels[-1]
            out_channels = tpn_out_channels

            if temporal_modulation_cfg is not None:
                scale = temporal_modulation_cfg['scales'][i]
            ...
            if i < self.num_tpn_stages - 1:
                ...
                if downsample_cfg is not None:
                    self.downsample_ops.append(out_channels, out_channels,
                                               **downsample_cfg)

        out_channels = level_fusion_cfg.out_channels
        ...
        if aux_head_cfg is not None:
            self.aux_head = AuxHead(self.tpn_in_channels[-2], **aux_head_cfg)
        else:
            self.aux_head = None

    def foward(self, x, target=None):
        loss = None

        # Auxiliary loss
        if self.aux_head is not None:
            loss = self.aux_head(x[-2], target)
        ...
        # Build top-down flow using upsample operation
        outs = copy.deepcopy(temporal_modulation_outs)
        ...
        if self.flow_type == 'cascade':
            outs = copy.deepcopy(top_down_outs)
        else:
            outs = copy.deepcopy(temporal_modulation_outs)
