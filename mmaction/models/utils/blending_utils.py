...
class CutmixBlending(BaseMiniBatchBlending):
    ...
    def rand_bbox(img_size: torch.Size, lam: torch.Tensor) -> Tuple:
        ...
        bbx1 = torch.clamp(cx - torch.div(cut_w, 2, rounding_mode='trunc'), 0,
                           w)
        bby1 = torch.clamp(cy - torch.div(cut_h, 2, rounding_mode='trunc'), 0,
                           h)
        bbx2 = torch.clamp(cx + torch.div(cut_w, 2, rounding_mode='trunc'), 0,
                           w)
        bby2 = torch.clamp(cy + torch.div(cut_h, 2, rounding_mode='trunc'), 0,
                           h)
