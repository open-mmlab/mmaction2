...
class BSNSSNDataset(BaseDataset):
    ...
    def load_annotations(self):
        ...
        for proposal_info in proposal_infos:
            if self.data_prefix is not None:
                frame_dir = osp.join(self.data_prefix, proposal_info[0].split('-')[0], 'frames')
            
            #num_frames = min(int(proposal_info[1])/2 * int(proposal_info[0].split('-')[1]) + int(proposal_info[1]), len(os.listdir(frame_dir)))
            
            
            clip_offset = int(proposal_info[1])//2 * int(proposal_info[0].split('-')[1])
            num_frames = min(clip_offset + int(proposal_info[1]), len(os.listdir(frame_dir)))
            
            assert clip_offset < num_frames
            #num_frames = num_frames-clip_offset
            ...
            video_infos.append(
                dict(
                    frame_dir=frame_dir,
                    hshi_offset=clip_offset,
                    video_id=proposal_info[0],
                    total_frames=num_frames - clip_offset,
                    gts=gts,
                    proposals=proposals))
