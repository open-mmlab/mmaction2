...
def train_model_xusu(model,
    ...
    if cfg.get('precise_bn', False):
        ...
        print('finish register precise bn hook')

    # multigrid setting
    multigrid_cfg = cfg.get('multigrid', None)
    if multigrid_cfg is not None:
        from mmaction.utils.multigrid import MultiGridHook
        multigrid_scheduler = MultiGridHook(cfg)
        runner.register_hook(multigrid_scheduler, priority='HIGH')
        print('Finish register multigrid hook')
        ...
        print('Finish register subbn3daggre hook')
